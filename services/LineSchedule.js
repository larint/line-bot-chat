...
LineSchedule.run = async () => {
    ...
    Promise.all(accountList.map(async (account) => {
        ...
        let dateRange = [];
        if (maxDate) {
            dateRange = helper_1.getDateRangeLimitNow(maxDate, 15, 'YYYY-MM-DD');
        }
        else {
...
LineSchedule.saveMessageStatistic = async (client, accountId, dateUpdate) => {
    ...
    let apiDate = helper_1.formatDate('YYYYMMDD', new Date(dateUpdate));
    let messageStatistic = new messages_statistic_1.MessageStatistic();
    let result = {};
    try {
        ...
        result.messageDeliveries = await client.getNumberOfMessageDeliveries(apiDate);
        let follower = { status: 'unready', blocks: 0, targetedReaches: 0, followers: 0, block_rate: 0 };
        follower = await client.getNumberOfFollowers(apiDate);
...
LineSchedule.saveFollowerStatistic = async (client, accountId) => {
    let apiDate = helper_1.formatDate('YYYYMMDD', new Date(), -1);
    let follower = { status: 'unready', blocks: 0, targetedReaches: 0, followers: 0, block_rate: 0 };
    try {
        follower = await client.getNumberOfFollowers(apiDate);
        let blocks = follower.blocks;
